...
def from_didl_string(string):
    ...
    for elt in root:
        if elt.tag.endswith('item') or elt.tag.endswith('container'):
            ...
            try:
                if item_class in _DIDL_CLASS_TO_CLASS:
                    cls = _DIDL_CLASS_TO_CLASS[item_class]
                elif item_class.find(".#") > -1:
                    print(item_class[:item_class.find(".#")])
                if item_class[:item_class.find(".#")] in _DIDL_CLASS_TO_CLASS:
                    cls = _DIDL_CLASS_TO_CLASS[item_class[:item_class.find(".#")]]
            except KeyError:
...
class DidlObject(with_metaclass(DidlMetaClass, object)):
    ...
    def from_element(cls, element):     # pylint: disable=R0914
        ...
        if item_class.find(cls.item_class) != 0:
            raise DIDLMetadataError(
